<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="文件后缀" prop="imgSuffix">
        <el-input
          v-model="queryParams.imgSuffix"
          placeholder="请输入文件后缀"
          clearable
          size="small"
    ...
    <el-table v-loading="loading" :data="imgList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" prop="imgId" />
      <el-table-column label="文件后缀" align="center" prop="imgSuffix" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="文件后缀" prop="imgSuffix">
          <el-input v-model="form.imgSuffix" placeholder="请输入文件后缀" />
        </el-form-item>
        <el-form-item label="图片名" prop="imgName">
...
<script>
import { listImg, getImg, delImg, addImg, updateImg, exportImg } from "@/api/fileimg/img";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除图片文件编号为"' + imgIds + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有图片文件数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
